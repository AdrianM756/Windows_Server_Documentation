...
Based on the output, we can see that the account is Locked out. To enable her account, We will use the command: ```UnlockADAccount -Identity <username>```. Once done, Let us verify and check her account status. As you can see, the Lock Out status is now set to ``No``.
